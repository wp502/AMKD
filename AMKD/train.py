...
from distillers.amkd import compute_amkd_loss
...
def train_model(args, train_loader, val_loader, num_category,
    ...
    need_teacher = args.distiller in {'msd', 'dsmd', 'kdmcse', 'g2d', 'dclip', 'amkd'}
    if need_teacher:
    ...
    for epoch in range(total_epochs):
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
            ...
            if args.distiller in {'msd', 'dsmd', 'kdmcse', 'g2d', 'dclip', 'amkd'}:
            ...
            if num_category:
                ...
                elif args.distiller == 'amkd':
                    loss_distill = compute_amkd_loss(outputs_t1, outputs_t2, outputs_s, batch.get("label", None), device=images.device.type, config=args)
                else:
