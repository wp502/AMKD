...
def _forward_logits(model, batch, device, prefer="auto", fuse_alpha=None):
    """
    返回用于评估的 logits:
    - prefer="auto": 优先 joint_logits，其次 late-fusion(text+image)
    - prefer="joint" / "text" / "image"
    - fuse_alpha: 当使用 late-fusion 时，logits = a*text + (1-a)*image
    """
...
def _fit_temperature_lbfgs(logits, labels, init_T=1.0, max_iter=50):
    """用 LBFGS 最小化 BCE(logits/T, labels)，返回标定温度 T>0"""
...
def _per_class_thresholds(probs, labels, grid=None):
    """在验证集上做逐类阈值搜索，最大化每类F1；返回 thresholds [C]"""
    if grid is None:
...
def evaluate_classification(
    ...
    val_loader,
    dataset: str = None,
    thresholds: np.ndarray = None,      # 逐类阈值 [C]；若为 float 则当作全局阈值
    topk: int = 1,
    prefer: str = "auto",               # 'auto'|'joint'|'text'|'image'
    fuse_alpha: float = None,           # late-fusion 的 text 权重 α（auto 且无 joint 时生效）
    calibrated: bool = True,            # 是否做温度标定+逐类阈值搜索
    return_calib: bool = True,          # 返回 T/thresholds/alpha（便于复用）
):
    """
    - VQA：soft accuracy（与原逻辑一致）
    - 多标签：支持 late-fusion、温度标定、逐类阈值；默认更利于 Macro-F1
    """
    model.eval()
...
def evaluate_retrieval(
    ...
    max_text_templates: int = 4,        # 上限，防止过多模板导致显存激增
):
    model.eval()
    device = _get_device(model)
    ...
    # --------- 文本模板集成：按模板分别编码文本，再均值 ---------
    def _encode_text_ensemble(texts: list, template_list: list):
        """
        texts: list[str]，长度 = B_caption
        template_list: list[str]，包含 "{}" 占位
        return: [B_caption, D] (CPU tensor, normalized)
        """
